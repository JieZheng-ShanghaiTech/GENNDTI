...
parser.add_argument('--dataset', type=str, default='davis', help='which dataset to use,davis or KIBA')
parser.add_argument('--split', type=int, default=0, help='by which way we construct the dataset,0,1,2,3 for s1,s2,s3,s4,respectively')
...
parser.add_argument('--inner_model', type=int, default=0, help='0 for default, 1 for gcn, 2 for gat, 3 for gin')
parser.add_argument('--cross_model', type=int, default=0, help='0 for default, 1 for gcn, 2 for gat, 3 for gin')
...
for split in range(1):
    for cross in range(1):
        for inner in range(1,2):
            args.split = split
            args.inner_model = inner
            args.cross_model = cross
            
            #dataset = Dataset_larger('../data/', args.dataset, args.rating_file, sep, 10)
            print(f'using split type {split}')
            print(f'usong inner type {inner}')
            print(f'using cross tyoe {cross}')
